...
class StockBotApp:
    def __init__(self, root: tk.Tk, settings: Settings) -> None:
        ...
        # The date whose data is actually on screen ("" = latest view). Kept
        # separate from the entry text so a failed lookup never strands the
        # table on an empty date.
        self.active_date = ""
    ...
    def _configure_style(self) -> None:
        ...
        style.map("Treeview", background=[("selected", "#cfe2ff")], foreground=[("selected", "#0b1b33")])
    ...
    def _build_layout(self) -> None:
        ...
        summary = ttk.Frame(self.root, padding=(14, 2, 14, 12))
        ...
        tile_colors = {
            "date": ("#eef3fb", "#1f3a5f"),
            "count": ("#eef3fb", "#1f3a5f"),
            "up_down": ("#eef3fb", "#1f3a5f"),
            "top_up": ("#fdeceb", "#c62828"),
            "top_down": ("#e9f1fb", "#1565c0"),
        }
        for index, key in enumerate(["date", "count", "up_down", "top_up", "top_down"]):
            background, foreground = tile_colors[key]
            tk.Label(
                summary,
    ...
    def sync_prices(self) -> None:
        ...
        if not symbols:
            self._show_dialog("warning", "확인", "관심종목을 한 개 이상 입력해주세요.")
            return
        target = base_date or ""
        ...
        def done(count: int) -> None:
            self.active_date = target
            self.base_date_var.set(_format_date(target) if target else "")
            self.status_var.set(f"{count}개 관심종목을 업데이트했습니다.")
            self.refresh_table()

        def on_error() -> None:
            # Lookup failed (e.g. no data for that day): keep showing the last
            # good date instead of stranding the table on an empty filter.
            self.base_date_var.set(_format_date(self.active_date) if self.active_date else "")

        self._run_background(task, done, on_error)
    ...
    def refresh_table(self) -> None:
        ...
        if self.active_date:
            rows = [row for row in all_rows if _date_digits(row.get("base_date")) == self.active_date]
        else:
        ...
        for index, row in enumerate(rows):
            ...
            self.tree.insert("", "end", values=_row_values(row), tags=(_row_tag(row), stripe))
    ...
    def _run_background(
        ...
        def worker() -> None:
            try:
                ...
                self.root.after(0, lambda exc=exc: self._handle_error(exc, on_error))
            else:
    ...
    def _handle_error(self, exc: Exception, on_error: Callable[[], None] | None) -> None:
    ...
    def _show_error(self, exc: Exception) -> None:
        ...
        self._show_dialog("error", "오류", message)
    ...
    def _show_dialog(self, kind: str, title: str, message: str) -> None:
        ...
        body = tk.Frame(top, bg="#ffffff", padx=30, pady=26)
        body.pack(fill="both", expand=True)
        body.columnconfigure(1, weight=1)

        tk.Label(body, text=icon, font=("맑은 고딕", 34), bg="#ffffff").grid(
            row=0, column=0, rowspan=2, padx=(0, 20), sticky="n"
        )
        tk.Label(
            body,
            text=title,
        ...
        tk.Label(
            ...
            anchor="w",
            wraplength=470,
        ).grid(row=1, column=1, sticky="w", pady=(10, 20))

        confirm = ttk.Button(body, text="확인", style="Primary.TButton", command=top.destroy)
